...
| Option | Meaning |
|---|---|
| `--product` | Display name — the human-facing name shown in the wizard, Add/Remove Programs (`DisplayName`), version-info, and shortcut labels. |
| `--product-id` | Registry-safe internal id, distinct from `--product`. Drives the HKCU Uninstall key, association ProgIDs, the per-user data folder and upgrade detection. Must match `^[A-Za-z][A-Za-z0-9._-]{0,49}$`; keep it **stable across versions**. |
